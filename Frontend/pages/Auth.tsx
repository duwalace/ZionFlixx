...
import { Film, Mail, Lock, ArrowRight, ArrowLeft } from 'lucide-react';
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-background relative">
        ...
        <div className="w-full max-w-md bg-surface p-10 rounded-2xl border border-white/10 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {loading ? 'Carregando...' : mode === 'login' ? 'Entrar' : 'Enviar Link'}
          ...
          <div className="mt-8 text-center text-sm text-gray-400">
            {mode === 'login' && (
              <>
                <p className="mb-2">
                  <button onClick={() => toggleMode('forgot')} className="hover:text-brand transition-colors">Esqueceu a senha?</button>
                </p>
                <p>
                  Novo por aqui?{' '}
            ...
            {mode === 'forgot' && (
              <p>
                Lembrou sua senha?{' '}
                <button onClick={() => toggleMode('login')} className="text-brand font-bold hover:underline">
                  Voltar para login
                </button>
              </p>
            )}
