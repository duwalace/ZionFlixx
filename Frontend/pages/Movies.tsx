...
const Movies: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const movieList = convertedMovies.filter(m => m.type === 'movie');
        setAllMovies(movieList);
        setError(null);
      } catch (err) {
        console.error('Erro ao buscar filmes:', err);
        setError('Erro ao carregar filmes. Verifique se o backend está rodando.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (ageFilter === 'all') {
      ...
      setMovies(allMovies.filter(m => m.rating === ageFilter));
    }
  }, [ageFilter, allMovies]);
    ...
    <div className="min-h-screen bg-background text-white pb-20 pt-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          <h1 className="text-3xl font-bold text-brand">Filmes</h1>
        ...
        {movies.length === 0 ? (
          <div className="text-center text-gray-400 py-20">
            {ageFilter === 'all' 
              ...
              : `Nenhum filme encontrado com classificação ${ageFilter === 'L' ? 'Livre' : ageFilter + ' anos'}.`}
          </div>
        ) : (
